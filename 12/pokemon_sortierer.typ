...
$ (sum^(log_3(n))_(i=0) (2/3)^i dot n) + T(0) dot n^(log_3(2)) $

Mit $T(1) = Theta(1)$ erhalten wir 
$ (sum^(log_3(n))_(i=0) (2/3)^i dot n) + o(n) $
Jetzt ziehen wir $n$ aus der Summe:
$ n dot (sum^(log_3(n))_(i=0) (2/3)^i) + o(n) $
Diesen Term können wir abschätzen:
$ n dot (sum^(log_3(n))_(i=0) (2/3)^i) + o(n) < n dot (sum^(infinity)_(i=0) (2/3)^i) + o(n) $
